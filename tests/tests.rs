...
const TWELVE: &str = "illness spike retreat truth genius clock brain pass fit cave bargain toe";
const TWELVE_WITH_SPACES: &str =
    " illness spike retreat truth genius clock  brain pass fit cave   bargain toe ";

#[test]
pub fn test_1() {
    let test = |s: &str| {
        full_test(s.parse().unwrap(), None, "e4a5a632e70943ae7f07659df1332160937fad82587216a4c64315a0fb39497ee4a01f76ddab4cba68147977f3a147b6ad584c41808e8238a07f6cc4b582f186", 
    ...
    test(TWELVE);
    test(TWELVE_WITH_SPACES);
}

#[test]
pub fn test_2() {
    full_test("resource asthma orphan phone ice canvas fire useful arch jewel impose vague theory cushion top".parse().unwrap(),
        None,
...
#[test]
fn test_3_24_words() {
    full_test("bench hurt jump file august wise shallow faculty impulse spring exact slush thunder author capable act festival slice deposit sauce coconut afford frown better".parse().unwrap(),
    None,
...
#[test]
fn test_4_24_words_and_bip_39_passphrase() {
    let pass = Some("p4ssphr4se");

    full_test("cable spray genius state float twenty onion head street palace net private method loan turn phrase state blanket interest dry amazing dress blast tube".parse().unwrap(),
    pass,
...
#[test]
fn test_5_12_world_entry_bytes_00000000000000000000000000000000() {
    let seed_phrase = SeedPhrase::from_entropy(&[0; 16]).unwrap();
    let other: SeedPhrase = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about".parse().unwrap();
    assert_eq!(seed_phrase.phrase(), other.phrase());
    full_test(seed_phrase,
    None,
...
fn full_test(
    ...
    assert_eq!(format!("{seed:02x}"), bip39_seed);
    let primary_key = &phrase.empty_key(passphrase).unwrap().private().0;
    assert_eq!(primary_key.to_lower_hex(), empty_key);
    for (i, (public_key, private_key)) in hcnet_keys.iter().enumerate() {
